...
async fn incident_exchange(
    ...
    let mut config = ClientConfig::builder_with_details(provider.clone(), Arc::new(FixedTime(at_ms)))
        ...
        .with_no_client_auth();
    // The marker only steers the gateway; the monitor's own TLS server does
    // not list it, and TLS 1.3 aborts a handshake with no protocol in
    // common. `http/1.1` is what the two actually agree on.
