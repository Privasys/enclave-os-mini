...
//!    so a config is never sealed under a KEK the constellation cannot give
//!    back.
...
//!
//! The KEK never leaves TEE memory and the platform never holds it. It wraps
//! the app's KV master-key (see [`crate::sealed_config`]).
...
fn shamir_split(secret: &[u8], threshold: usize, num_shares: usize) -> Result<Vec<Share>, String> {
    ...
    let mut shares: Vec<Share> = (0..num_shares)
        ...
        .collect();
    let rng = SystemRandom::new();
    let mut coeffs = vec![0u8; threshold - 1];
    for &b in secret {
