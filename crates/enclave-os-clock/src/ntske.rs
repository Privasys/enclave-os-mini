...
//! one request (NTPv4 as next protocol, AEAD_AES_SIV_CMAC_256, end of
//! message); the server answers with the same negotiation, a set of opaque
//! cookies and optionally another NTP host or port. The two AEAD keys are
//! not sent: both sides derive them from the TLS exporter
...
/// IANA id of AEAD_AES_SIV_CMAC_256.
pub const AEAD_AES_SIV_CMAC_256: u16 = 15;
/// Key length of AEAD_AES_SIV_CMAC_256 (two AES-128 keys).
...
/// The exporter context for one direction: protocol id, AEAD id, then
/// 0x00 for client-to-server or 0x01 for server-to-client (§5.1).
pub fn exporter_context(server_to_client: bool) -> [u8; 5] {
    let p = PROTOCOL_NTPV4.to_be_bytes();
    let a = AEAD_AES_SIV_CMAC_256.to_be_bytes();
...
/// The client's request: NTPv4, AEAD_AES_SIV_CMAC_256, end of message.
pub fn build_request() -> Vec<u8> {
    let mut out = Vec::with_capacity(16);
    push_record(&mut out, true, REC_NEXT_PROTOCOL, &PROTOCOL_NTPV4.to_be_bytes());
    push_record(&mut out, true, REC_AEAD, &AEAD_AES_SIV_CMAC_256.to_be_bytes());
...
pub enum KeError {
    ...
    Protocol,
    /// The response did not negotiate AEAD_AES_SIV_CMAC_256.
    Aead,
    /// No cookie.
    NoCookies,
...
impl core::fmt::Display for KeError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            KeError::Aead => write!(f, "AEAD_AES_SIV_CMAC_256 not negotiated"),
...
pub fn parse_response(buf: &[u8]) -> Result<Option<KeResponse>, KeError> {
    ...
    loop {
        ...
        match rtype {
            ...
            REC_AEAD => {
                ...
                aead = Some(body == AEAD_AES_SIV_CMAC_256.to_be_bytes());
            }
            REC_NEW_COOKIE => {
                if body.is_empty() {
    ...
    if aead != Some(true) {
        return Err(KeError::Aead);
    }
    if cookies.is_empty() {
        return Err(KeError::NoCookies);
    }
    Ok(Some(KeResponse { cookies, server, port }))
}
...
mod tests {
    ...
    fn response(extra: &[(bool, u16, &[u8])]) -> Vec<u8> {
        let mut out = Vec::new();
        push_record(&mut out, true, REC_NEXT_PROTOCOL, &[0, 0]);
        push_record(&mut out, false, REC_AEAD, &[0, 15]);
        for (c, t, b) in extra {
    ...
    fn request_bytes() {
        assert_eq!(
            build_request(),
            vec![0x80, 0x01, 0x00, 0x02, 0x00, 0x00, 0x80, 0x04, 0x00, 0x02, 0x00, 0x0f, 0x80, 0x00, 0x00, 0x00]
        );
    }

    #[test]
    fn exporter_contexts() {
        assert_eq!(exporter_context(false), [0, 0, 0, 15, 0]);
        assert_eq!(exporter_context(true), [0, 0, 0, 15, 1]);
    }

    #[test]
    fn parses_cookies_server_and_port() {
        let buf = response(&[
    ...
    fn incomplete_until_end_of_message() {
        let buf = response(&[(false, REC_NEW_COOKIE, b"c")]);
    ...
    fn refusals() {
        ...
        let buf = response(&[(true, 0x0042, b"x"), (false, REC_NEW_COOKIE, b"c")]);
        assert_eq!(parse_response(&buf), Err(KeError::UnknownCritical(0x42)));

        let buf = response(&[]);
        assert_eq!(parse_response(&buf), Err(KeError::NoCookies));

        let mut wrong_aead = Vec::new();
        push_record(&mut wrong_aead, true, REC_NEXT_PROTOCOL, &[0, 0]);
        push_record(&mut wrong_aead, true, REC_AEAD, &[0, 30]);
        push_record(&mut wrong_aead, false, REC_NEW_COOKIE, b"c");
        push_record(&mut wrong_aead, true, REC_END, &[]);
        assert_eq!(parse_response(&wrong_aead), Err(KeError::Aead));
    ...
    fn keeps_at_most_eight_cookies() {
        ...
        let r = parse_response(&response(&many)).unwrap().unwrap();
