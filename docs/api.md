...
1. `roles` (string array — standard RFC 9068)
2. `realm_access.roles` (Keycloak format)
3. Any configured map-shaped role claim (`{ "role-name": {...} }`)
...
| `token` | string? | Bearer token for authenticated servers (optional) |
