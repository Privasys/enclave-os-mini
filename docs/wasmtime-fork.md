...
On the enclave side, `src/sgx_platform.rs` in the `enclave-os-wasm` crate
provides the `extern "C"` symbols that wasmtime's `sys::custom::capi` module
expects. Key design choices:
